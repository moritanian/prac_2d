   ...
   // _txtCost.Label = "";
    if (selMode == GameMgr.eSelMode.Buy)
    {
     ...
     // _txtCost.SetLabelFormat("(cost ${0})", cost);
      _btnBuy.FormatLabel("cost \n (${0})", cost);
    }else{
      // 購入ボタンを押せるかどうかチェック
      _btnBuy.Enabled = (Global.Money >= cost);
    }
    // 購入コストを表示する
    _btnBuy.FormatLabel("Buy \n (${0})", cost);
    // ライフ表示
    ...
    if (selMode == GameMgr.eSelMode.Upgrade)
    {
      ...
      */
		for( int i = 0; i< Global.TowerParMax; i++){
			bool b = (tower.LvRange>i);
			MyCanvas.SetActive ("RangeLv" + (i+1),b);
			b = (tower.LvFirerate>i);
			MyCanvas.SetActive ("FirerateLv" + (i+1),b);
			b = (tower.LvPower>i);
			MyCanvas.SetActive ("PowerLv" + (i+1),b);
		}
