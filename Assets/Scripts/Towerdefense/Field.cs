...
public class Field : Token
{
  ...
  //座標取得系メソッドは、x方向にマップ画像とカメラの左端一致、y方向にマップ画像、カメラとも中心が０として計算している。
  ...
  public static float ToWorldX(int i)
  {
    ...
    //Vector2 min = Camera.main.ViewportToWorldPoint(new Vector2(0, 0));
		//Debug.Log (min.x.ToString());
    var spr = GetChipSprite();
    var sprW = spr.bounds.size.x;
    //Debug.Log(sprW);
    return (sprW * i) - ((float)CreateMap.CHIP_WIDTH/2.0f * sprW) + sprW/2.0f; 
  ...
  public static float ToWorldY(int j)
  {
    ...
    return ((float)CreateMap.CHIP_HEIGHT * sprH/2.0f) - (sprH * j) -sprH/2.0f;
  }
  ...
  public static int ToChipX(float x)
  {
   // Vector2 min = Camera.main.ViewportToWorldPoint(new Vector2(0, 0));
    ...
    return (int)((CreateMap.CHIP_WIDTH * sprW /2 + x) / sprW);  
  }
  ...
  public static int ToChipY(float y)
  {
   ...
   // return (int)((y - max.y) / -sprH);
    return (int)((y - (CreateMap.CHIP_HEIGHT * sprH) /2 ) / -sprH);
  }

  public void Load()
	{
		if(Settings.map_create){ //作成
      LoadCreate();
    }else{
      LoadMap();
    }
  ...
  void LoadCreate(){
    // マップ作って必要なレイヤーを生成
    Layer2D Map = CreateMap.Create();
    // マップのクローンを作製
    _lCollision = Map.Clone(); 
    _lCollision.Dump();
    // 座標リスト
    _path = new List<Vec2D>();
   // 開始位置
    Vec2D pos = Map.Search(CreateMap.CHIP_PATH_START);
    _path.Add(new Vec2D(pos.x,pos.y));
    // パス作成のために開始位置をけす
    Map.Set(pos.X ,pos.Y, CHIP_NONE);
    // 座標リスト作成
    CreatePath(Map, pos.X, pos.Y, _path);
