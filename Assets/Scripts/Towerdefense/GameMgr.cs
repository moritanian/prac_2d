...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/12/05 01:24:51 by anonymous         #+#    #+#             */
/*   Updated: 2016/03/24 22:01:46 by anonymous        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
  ...
  void ChangeSelMode(eSelMode mode)
  {
    switch (mode)
    {
      ...
      case eSelMode.Buy:
        ...
        if(MyCanvas.Find<ButtonObj>("ButtonBuy")){
            MyCanvas.Find<ButtonObj>("ButtonBuy").Enabled = false;
            Debug.Log("Successfully Button");
        }else Debug.Log("ButtonFalse");
        // タワー情報は非表示
        MyCanvas.SetActive("TextTowerInfo", false);
        // 射程範囲を非表示
        _cursorRange.SetVisible(false, 0);
      ...
      case eSelMode.Upgrade:
        ...
        // 購入ボタンを表示する
        MyCanvas.SetActive("ButtonBuy", true);
        //MyCanvas.Find<ButtonObj>("TextTowerInfo").Enabled = false;
        // タワー情報を表示
