...
public class Util {
  ...
  /// Mathf.Sinの角度指定版.
	...
	/// 指定した2つのTokenの距離を取得する
 	public static float Distance3DBetween(Token3D t1, Token3D t2) {
    	var dx = t2.X - t1.X;
    	var dy = t2.Y - t1.Y;
    	var dz = t2.Z - t1.Z;
  ...
  public static bool GUIButton(float x, float y, float w, float h, string text) {
	...
	/// ゲームデータを保存する
	public static bool SaveData(Hashtable inthashdata, Hashtable strhashdata, Hashtable fhashdata){
		PlayerPrefs.SetString("a","kk");
		foreach(DictionaryEntry datum in inthashdata){
			PlayerPrefs.SetInt(datum.Key.ToString(), int.Parse(datum.Value.ToString()));
		}
		foreach(DictionaryEntry datum in strhashdata){
			PlayerPrefs.SetString(datum.Key.ToString(), datum.Value.ToString());
		}		
		foreach(DictionaryEntry datum in fhashdata){
			PlayerPrefs.SetFloat(datum.Key.ToString(), float.Parse(datum.Value.ToString()));
		}	
